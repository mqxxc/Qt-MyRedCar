...
RcWidget::RcWidget(QWidget* parent) : QFrame(parent)
{
//其他初始化
	InitMember();
}
...
bool RcWidget::AddUnit(QString path)
{
	...
	QString filePath = YQTools_Qt::ToAbsolutePath(dataPath + name);			//以视频名字作为文件夹名字
	QDir dir;
	...
	QLibrary dll(YQTools_Qt::ToAbsolutePath("/dll/Screenshot"));
	if (!dll.load())
