...
{
	return YQTools_Qt::ToAbsolutePath(recordPath + m_strName + "/" + m_strPhoto);
}

QString UnitMsg::GetVideoPath()
{
	return YQTools_Qt::ToAbsolutePath(recordPath + m_strName + "/" + m_strVideo);
}
...
{
	QFile file(YQTools_Qt::ToAbsolutePath(recordPath + m_strName));
	if (!file.exists())
	{
		return false;
	}

	return file.rename(YQTools_Qt::ToAbsolutePath(recordPath + strNewName));
}